...
#Autor: WME-Kyndryl Colombia 2025 Julio Cesar Rodriguez Ibague
#Version:v1
...
#CAMBIO={{cambio}}
#SERVER={{servidor}}
#BAR_FILES="{{aplicaciones}}" #arreglo separado con espacio.

#CREA_GE={{crea_ge}}
#NODE_BOKER={{nombre_broker}}
#NOM_GE={{nom_ge}}

#CREA_MQ={{crea_mqc}}
#NOM_MQ={{nom_gestorMQ}}
#SCRIPT_COLA={{script_colaMQ}}

#LISTA_MAIL="{{emails}}"
#RUTA_SOA="/var/mqm/deploy/$CAMBIO" #/home/deployuser/deploy/$CAMBIO acá se ubucaran los bar y los script para creacion de las colas

CAMBIO=CRQ000000XXXX70
SERVER=CDPAIX10
BAR_FILES="FIN_FID_INT_CANALES_ST;aplicacion111.bar;1;IIBQA BANCOOMEVA;aplicacion2.bar;0;IIBQA-verifcando FastTicketing;aplicacion3.bar;1;IIBQA-verificando "
CREA_GE=1
NODE_BOKER=IIBQA
NOM_GE=grupo-ejecucion1
CREA_MQ=1
NOM_MQ=WMQQA
SCRIPT_COLA=scripts.mqsc
RUTA_SOA="/var/mqm/deploy/$CAMBIO" #/home/deployuser/deploy/$CAMBIO
LISTA_MAIL="juliorod@kyndryl.com jhonfreddy.osorio@kyndryl.com edwin.giraldo@kyndryl.com"
...
mqsicreateexecutiongroup "$BROKER_NAME" -n "$EXECUTION_GROUP_NAME"
...
mqsideploy "$INTEGRATION_NODE" -e "$IS" -a "$BARFILE" -m
exit_code=$?
if [ "$exit_code" -eq 0 ]; then
