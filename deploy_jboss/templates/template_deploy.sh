...
lista_mail="{{emails}}"
cambio={{cambio}}
ruta="/home/deployuser/deploy/$cambio"
ruta_log="/home/deployuser/logs"
ruta_bin="/home/deployuser/bin"
reinicio={{reinicio}}
...
validacion() {
  if [[ $version == "jboss-as-7.1.1.Final" || $version == "jboss-eap-7.2" || $version == "jboss-eap-7.1" ]]; then
    ...
    if [ -n "$pid" ]; then
	    echo "validacion::: hay una instancia activa para la aplicacion"
     # break
    else
      su - "$user" -c "sh $JBOSS_DOMAIN/bin/$script" >/dev/null 2>&1 &
    ...
    pid=$(pgrep -f "java.*$JBOSS_DOMAIN.*$instancia")
    if [ -n "$pid" ]; then
	    echo "validacion::: hay una instancia activa para jboss 4.4"
      break
    else
      su - "$user" -c "sh $JBOSS_DOMAIN/bin/$script" >/dev/null 2>&1 &
      sleep 2
...
restart_j4() {
  ...
  if [ -n "$pid" ]; then
    ...
    su - "$user" "sh $JBOSS_DOMAIN/bin/$script" >/dev/null 2>&1 &
    sleep 2
    last_line=$(cat -n "$JBOSS_HOME"/log/server.log | tail -n 1 | awk '{print $1}')
    while true; do
      linea=$(tail -n +"$((last_line + 1))" "$JBOSS_HOME"/log/server.log | grep -Eo "Started in|Starting failed")
  ...
  while true; do
    ...
    if [[ -n "$deploy_deployed" ]]; then
      ...
      if [[ $retry_count -eq 1 ]]; then
        printf -v asunto_rollback "[Rollback Ejecutado] %s aplicación %s" "$cambio" "$app"
...
" "$app" "$instancia" "$server" "$hora" "$listar"
        printf -v asunto_exito "[Despliegue Exitoso] %s aplicación %s" "$cambio" "$app"
...
" "$app" "$instancia" "$server" "$hora"
        printf -v asunto_fallo "[ERROR] %s despliegue fallido aplicación %s" "$cambio" "$app"
...
rollback() {
  ...
  rm -f "$dir_deploy/${app}"* 2>/dev/null
  sleep 5
  app_roll=$(ls "$dir_backup/${app}"* |tail -1)
   - -ppcp -p "$app_roll" "$dir_deploy/$app"
  chown "$user:$user" "$dir_deploy/$app"
  chmod 644 "$dir_deploy/$app"
...
realizar_despliegue() {
  ...
  if [[ -e "$dir_deploy/$app" ]]; then
    cp -p  "$dir_deploy/$app" "$dir_backup/${app}_${date}"
    sleep 5
...
for app in $apps; do
  ...
  if [ $existe -eq 0 ]; then
	  ...
	  if (( $(echo "$user" | wc -l) > 1 )) || (( user == 0 )); then
	    user=jboss
	  fi
  ...
  echo Llamando la funcion de 'validacion' para [$app]
  validacion
