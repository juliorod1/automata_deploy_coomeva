...
for server in $list_servers; do
  ...
  if [ $crea_e -eq 1 ]; then
	...
	echo
        ...
        read nom_e
        echo
	echo "**Si va a Desplegar un nuevo bar en el nuevo grupo de ejecucion recien creado, lo indicará en el siguiente paso**"
	echo
	echo "Adiciona grupo a listado de grupo de ejecucion disponibles para actualizar"
	echo "$nom_e    $nom_broker"
	echo "$nom_e	$nom_broker" >>$PROJECT_HOME/Lista_aplicaciones/$server.txt
	sleep 2
  ...
  echo
  echo "Nuevo grupo de ejecucion disponible para actualizacion:" 
  echo "**$nom_e    $nom_broker**"
  echo
  ...
  if [ $crea_mq -eq 1 ]; then
	...
	ls /home/scripts/automata_ansible/Broker/*.*
