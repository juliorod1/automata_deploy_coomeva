...
COMPARTIDO="/home/scripts/automata_ansible/microservicios"
HISTORICO="/home/scripts/automata_ansible/microservicios/$servidor/historico_ejecutado"
PROJECT_HOME="/ansible"
INVENTORY="$PROJECT_HOME/hosts.cfg"
PROGRAMACION="/ansible/programacion/listado_programacion_micros.txt"
...
prog=1
clear
echo "			AUTOMATIZACION DESPLIEGUE APPLICACIONES COOMEVA-Microservicios Java"
echo "			==================================================="
echo
...
echo
echo " 				**LISTA DE CAMBIOS AGENDADOS PARA MICROSERVICIOS**								     "
echo
echo "======================================================================================================================="
...
select opcion in "${opciones[@]}"; do
   ...
   "Continuar") echo continuando con el proceso de cancelacion..
        ...
        *)
			...
			ansible-playbook -i $INVENTORY --limit $j $PROJECT_HOME/cancel-deploy_micros.yml --extra-vars "app_play=$c1"
		    done
...
for server in $list_servers; do
  ...
  opciones=(`cat $PROJECT_HOME/Lista_aplicaciones/$server.txt|awk 'BEGIN{ FS="|" } { print $1 }'` Continuar)
     ...
     for i in "${opciones[($REPLY-1)]}"; do
      ...
      if [ $REPLY  ]; then
         ...
         if [ "$i" != "Continuar" ] && [ "$i" != "" ]; then
            #set -x
	    echo -Es necesario reiniciar TODOS los jar del proyecto ["${opciones[($REPLY-1)]}"] ?...
	    echo
	    rein_jar=0
	    echo "* Digite 1 para confirmar el reinicio."
	    echo "* Digite 0 para no reiniciar."
	    read rein
	    if [ $rein -eq 1 ];then 
		rein_jar=1
	    fi
           # list_wars+="${opciones[($REPLY-1)]}"
	    s1=$(grep "${opciones[($REPLY-1)]}" $PROJECT_HOME/Lista_aplicaciones/$server.txt|awk 'BEGIN{ FS="|" } { print $2 }')
	   # list_scrip+="$s1 "
            list_wars+="${opciones[($REPLY-1)]};$s1;$rein_jar "
  ...
  ########
  echo
  echo at_datetime: '"'$fecha_hora'"' >$PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt
  echo cambio: $CR >>$PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt
  echo servidor: $server >>$PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt
  echo aplicaciones: '"'$list_wars'"' >>$PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt
  #echo script_reinicio: '"'$list_scrip'"' >>$PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt
  echo emails: '"'$email'"' >>$PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt
  cat $PROJECT_HOME/deploy_microservicios/vars/var_temp.txt > $PROJECT_HOME/deploy_microservicios/vars/main.yml
  cat $PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt >> $PROJECT_HOME/deploy_microservicios/vars/main.yml
  mv $PROJECT_HOME/deploy_microservicios_"$CR"_"$server".txt $PROJECT_HOME/historico/
  cd $PROJECT_HOME
 # echo esta linea ejecutara..
  ##ansible-playbook --limit=$server  deploy-jboss.yml --extra-vars "cambio=$CR apps=$list_wars servidor=$server reinicio=$rein at_datetime=$at_tiempo"
   ...
   cat $PROJECT_HOME/historico/deploy_microservicios_"$CR"_*.txt #$PROJECT_HOME/historico/cancelado_"$CR".txt
   echo
   ...
   if [ $nexts == 1 ]; then
      ...
      ansible-playbook -i $INVENTORY --limit=$server  deploy-micros.yml 
	...
	echo FALLA LA PROGRAMACION DE ESTE CAMBIO POR FALLAS EN LA EJECUCION DE PLAYBOOK ANSIBLE
      ...
      mv $PROJECT_HOME/historico/deploy_microservicios_"$CR"_"$server".txt  $PROJECT_HOME/historico/cancelado_"$CR".txt
      break 2
   fi
done
clear
...
#===============Menu principal microservicios==============
while [ $prog  -eq 1 ]; do
echo "										Fecha actual: $(date)" # Muestra la fecha actual
#PS3="Elige tu opción: "
opciones=(			 "Gestionar_Microservicios" "Reinicios_Microservicios" "Otros" "Reportes" "Salir" )
PS3="				 Elige una opción Válida,o ENTER para continuar.."
select opt in "${opciones[@]}"
do
    case $opt in 

        "Gestionar_Microservicios") clear
			echo
			echo "			Has elegido Programar/Cancelar cambios de Microservicios java!!!! " 
			echo
			...
			select opt1 in "${opciones1[@]}"; do
				case $opt1 in
					"Programar Cambio") clear
					  ...
					  servidores=""
					  echo   "SERVIDORES DE MICROSERVICIOS DISPONIBLES"
					  echo
					  opciones=(`cat $PROJECT_HOME/hosts.cfg|grep microservicios|awk 'BEGIN{FS=" "} { print $1 }'` Continuar)
					  ...
					  ;;
				  "Cancelar Cambio")
					  echo "Vamos a realizar proceso de Reinicio de un Microservicio!!!"
					  cancela
					  ;;
				  "Salir")  break; clear;
					  ;; 
        ...
        "Reinicios_Microservicios") echo "Has elegido reinciar microservicios" 
                date; 
        ;;


	"Reportes") echo "Has Elegido generar un reporte de cambios programdos de Microservicios"
                date; 
        ;;
